...
const SINGLE_INSTANCE_MUTEX: &str = "Global\\SingBoxCommander_Weys_SingleInstance";
...
impl App {
    ...
    /// 启动 sing-box（含端口占用预检 + 自动换端口）。控制台按钮与托盘菜单共用。
    fn on_start(&self) {
        let st = STATE.get().unwrap();
        ...
        if let Some(what) = conflict {
            ...
            if AppState::port_in_use(default_api) {
                ...
                match st.regenerate_config() {
                    Ok(_) => {
                        log_line(&format!("端口 {} 被占用，已自动改用 代理 {} / API {}", what, new_port, new_api));
...
mod app_ui {
    ...
    impl nwg::NativeUi<AppUi> for App {
        fn build_ui(mut d: App) -> Result<AppUi, nwg::NwgError> {
            // 图标
            // 托盘图标内嵌进 exe（embed-resource），真正单文件，且走 LoadImageW 原生加载器，
            // 规避 source_bin(PNG) 在 Win11 下渲染成黑方块的 bug（nwg 把同一张位图同时当
            // hbmMask/hbmColor 且不做 alpha 预乘）。运行时用 EmbedResource::icon_str 取图标。
            let embed = nwg::EmbedResource::load(None)?;
            d.icon_green = embed.icon_str("GREEN", None).ok_or_else(|| {
                nwg::NwgError::resource_create(
                    "内嵌图标 GREEN 缺失（检查 embed.rc 与 nwg 的 embed-resource 特性）",
                )
            })?;
            d.icon_red = embed.icon_str("RED", None).ok_or_else(|| {
                nwg::NwgError::resource_create(
                    "内嵌图标 RED 缺失（检查 embed.rc 与 nwg 的 embed-resource 特性）",
                )
            })?;
...
fn main() {
    ...
    // 后台轮询：每秒状态 + 每30秒持久化，同时集成 watchdog 进程守护
    thread::spawn(move || {
        let mut fc: u32 = 0;
        loop {
            ...
            if !running {
                ...
                drop(s);
                st.wd_heartbeat_ok(); // 停止状态不累积失败
                continue;
            }
            let api_port = st.api_port();
            let traffic = status::fetch_traffic(api_port);
            let conns = status::fetch_connections(api_port);

            let mut api_ok = false;
            let mut s = st.status.lock().unwrap();
            if let Some((up, down)) = traffic { s.up_speed = up; s.down_speed = down; api_ok = true; }
            if let Some((ut, dt, c)) = conns { s.up_total = ut; s.down_total = dt; s.conns = c; st.mark_traffic_dirty(); api_ok = true; }
            drop(s);

            if api_ok {
                st.wd_heartbeat_ok();
            } else {
                let fails = st.wd_heartbeat_fail();
                if fails >= 3 {
                    st.push_log(&format!("watchdog 心跳失败 {} 次，尝试自动恢复", fails));
                    let (tried, msg) = st.wd_try_restart();
                    st.push_log(&msg);
                    if tried && msg.contains("已自动重启") && st.system_proxy.load(Ordering::Relaxed) {
                        // 自动重启后如原已开启系统代理，则恢复代理设置
                        if let Err(e) = st.set_system_proxy(true) {
                            st.push_log(&format!("watchdog 恢复系统代理失败：{}", e));
                        }
                    }
                }
            }
