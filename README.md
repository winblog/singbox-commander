...
- 🔗 **节点解析**：粘贴 `vless` / `vmess` / `Trojan` 等订阅链接，自动解析并管理节点列表
- 🔀 **两种接管模式**（互斥）
...
cargo build --release
# 产物：target/release/SingBoxCommander.exe
```
...
│   ├── status.rs      # 流量/连接轮询（直连 clash_api）
│   ├── green.ico      # 托盘绿色图标（构建期内嵌进 exe，无需随附分发）
│   └── red.ico        # 托盘红色图标（构建期内嵌进 exe，无需随附分发）
├── gen_icons.py       # 生成 green.ico / red.ico 托盘图标（多尺寸 32 位 alpha）
├── gen_app_icon.py    # 生成 app.ico 程序文件图标（绿色地球，含 256 档）
├── app.ico            # 程序文件图标（构建期内嵌进 exe，作为资源管理器 exe 图标）
├── embed.rc           # 资源定义：把 app.ico(ID=1) + green.ico/red.ico 编入 exe（单文件）
├── build.rs           # 构建脚本：调用 embed-resource 编译 embed.rc
├── Cargo.toml
├── Cargo.lock
├── LICENSE
└── .gitignore
